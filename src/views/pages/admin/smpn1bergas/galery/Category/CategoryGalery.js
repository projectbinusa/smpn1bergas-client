  ...
  const getAll = async () => {
    try {
      const response = await axios.get(
        `${API_DUMMY}/api/category_galery/all?page=${page - 1}&size=${rowsPerPage}`,
        {
          headers: {
      ...
      console.log("data galery: ", response);
      setPaginationInfo({
  ...
  const deleteData = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${API_DUMMY}/api/category_galery/` + id, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          })
                  ...
                  {filteredList.map((galery, no) => {
                    return (
                      <tr key={no}>
                        ...
                        <td data-label="Nama galery" className="text-long">
                          {galery.category}
                        </td>
                        <td data-label="Aksi">
                          <div className="aksi">
