  ...
  const getAll = async (pageNumber) => {
    try {
      const response = await axios.get(
        `${API_DUMMY}/api/foto_sarana/all/terbaru?page=${pageNumber - 1}&size=${rowsPerPage}&sortBy=id&sortOrder=desc`,
        {
          headers: {
      ...
      <div className="main-card box-tabel mb-3 card">
        ...
        <div
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
            <h6 style={{ margin: 0, fontWeight: "600" }}>Foto Sarana</h6>
              ...
              {filteredList.length > 0 ? (
                filteredList.map((berita, no) => (
                  <tr key={no}>
                    ...
                    <td data-label="Nama Sarana">{berita.sarana.nama_sarana}</td>
                    <td data-label="Tanggal Dibuat">{berita.createdDate}</td>
                    <td data-label="Tanggal Update">{berita.updatedDate}</td>
                    <td data-label="Aksi" className="action">
                      <div style={{ display: "flex", justifyContent: "center", gap: "8px" }}>
                        {/* Edit */}
                        <Link
                          to={`/edit-foto-sarana/${berita.id}`}
                          style={{
                            width: "34px",
                            height: "34px",
                            display: "flex",
